...
function findEnemy(){
    ...
    if(difficulty <= 3){//picks a random easy enemy if slider value is 3 or less
        randomEnemy = easyEnemies[Math.floor(Math.random() * easyEnemies.length)];
    }
    else if(difficulty > 3 && difficulty <= 6){//picks a random medium enemy if slider value is between 4 and 6
        randomEnemy = mediumEnemies[Math.floor(Math.random() * mediumEnemies.length)];
    }
    else if(difficulty > 6 && difficulty <= 9){//picks random hard enemy if slider value is between 7 and 9
        randomEnemy = hardEnemies[Math.floor(Math.random() * hardEnemies.length)];
    }
    else if(difficulty === 10){//picks random extreme enemy if slider value is 10
